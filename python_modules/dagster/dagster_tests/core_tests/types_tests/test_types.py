...
def test_input_type_returns_wrong_thing():
    ...
    with pytest.raises(
        ...
        match=re.escape(
            ...
            '<class \'int\'> which does not pass the typecheck for Dagster type BadType. Step '
    ...
    assert (
        ...
        'type <class \'str\'> when checking runtime value of type <class \'int\'>.'
    )
...
def test_output_type_returns_wrong_thing():
    ...
    assert (
        type_check_data.description
        == 'Type checks must return TypeCheck. Type check for type BadType returned value of type '
        '<class \'str\'> when checking runtime value of type <class \'int\'>.'
    )
